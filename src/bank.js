...
var Bank = /** @class */ (function () {
    ...
    Bank.prototype.addBranch = function (branch) {
        if (this.checkBranch(branch)) {
            console.log("this baranch \"".concat(branch.getName(), "\" already added "));
            return false;
        }
        this.branches.push(branch);
        console.log("".concat(branch.getName(), " was added to ").concat(this.name));
        return true;
    };
    Bank.prototype.addCustomer = function (branch, customer) {
        ...
        console.log("there is no ".concat(branch.getName(), "branch in ").concat(this.name, " bank "));
        return false;
    };
    Bank.prototype.addCustomerTransaction = function (branch, customerId, amount) {
        ...
        console.log(" there are no branch with name ".concat(branch.name));
        return false;
    };
    Bank.prototype.findBranchByName = function (branchName) {
        var branch = this.branches.filter(function (branch) { return branch.getName() == branchName; });
        if (branch.length == 0) {
            console.log("there is no branch with this name \"".concat(branchName, "\""));
            return null;
        }
        return branch;
    };
    ...
    Bank.prototype.listCustomers = function (branch, includeTransactions) {
        console.log("_____".concat(this.name, " bank ").concat(branch.getName(), " list_____"));
        if (this.checkBranch(branch)) {
            ...
            for (var _i = 0, customers_1 = customers; _i < customers_1.length; _i++) {
                ...
                console.log("         ".concat(customer.getId(), "   ").concat(customer.getName()));
...
var Branch = /** @class */ (function () {
    ...
    Branch.prototype.addCustomer = function (customer) {
        var condition = this.getCustomers().find(function (cust) { return cust.getId() == customer.getId(); });
        if (!condition) {
            this.getCustomers().push(customer);
            console.log("".concat(customer.getId(), ", ").concat(customer.getName(), " was added to ").concat(this.name));
            return true;
        }
        console.log("".concat(customer.getId(), ", ").concat(customer.getName(), " already added to ").concat(this.name));
        return false;
    };
    Branch.prototype.addCustomerTransaction = function (customerId, amount) {
        var index = this.getCustomers().findIndex(function (customer) { return customer.getId() == customerId; });
        if (index == -1) {
...
var Customer = /** @class */ (function () {
    ...
    Customer.prototype.addTransactions = function (amount) {
        if (this.getBalance() + amount >= 0) {
            var date = new Date();
            this.getTransactions().push({ amount: amount, date: date });
            console.log(" the transaction was processed successfully \n your current balance is ".concat(this.getBalance()));
            return true;
        }
        console.log(" There is no enough balance for this transaction \n your current balance is ".concat(this.getBalance()));
        return false;
    };
    return Customer;
}());
